...
class Auth extends CI_Controller
{
	...
	public function createAccount()
	{
		...
		if ($user_exists) {
			$this->session->set_flashdata('errorRegister', 'This account already exists with this username.');
			redirect(base_url());
			return;
		}
		...
		if ($this->User_model->create_user($data)) {
			$this->session->set_flashdata('successRegister', 'Account created successfully. You can now log in.');
			redirect(base_url());
		} else {
			$this->session->set_flashdata('errorRegister', 'Failed to create account. Please try again.');
			redirect(base_url());
		}
	}
	...
	public function login()
	{
		...
		if ($user && password_verify($password, $user->password)) {
			...
			redirect(base_url('index.php/dashboard')); // Assuming 'dashboard' as a protected page after login
		} else {
